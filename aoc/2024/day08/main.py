...
def get_antinodes_pt2(grid: Grid, loc1: Loc, loc2: Loc) -> list[Loc]:
    dy = loc2[0] - loc1[0]
    dx = loc2[1] - loc1[1]
    ...
    antinode_loc = loc1
    while within_grid(grid, antinode_loc):
        antinodes.append(antinode_loc)
        antinode_loc = (antinode_loc[0] - dy, antinode_loc[1] - dx)
...
def solve_p2(path: str):
    ...
    for id, nodes in nodes.items():
        ...
        for perm in permutations(nodes, 2):
            print(f"perm: {perm}")
            for a in get_antinodes_pt2(grid, perm[0], perm[1]):
                antinodes.add(a)
